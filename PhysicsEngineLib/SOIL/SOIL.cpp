	...
	)
{
	...
	QOpenGLFunctions *f = QOpenGLContext::currentContext()->functions();
	f->glGetIntegerv( texture_check_size_enum, &max_supported_size );
	...
	if( tex_id )
	{
		...
		QOpenGLFunctions *f = QOpenGLContext::currentContext()->functions();
		f->glBindTexture( opengl_texture_type, tex_id );
		...
		if( flags & SOIL_FLAG_MIPMAPS )
		{
			...
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		} else
		{
			/*	instruct OpenGL _NOT_ to use the MIPmaps	*/
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
			check_for_GL_errors( "GL_TEXTURE_MIN/MAG_FILTER" );
		}
		/*	does the user want clamping, or wrapping?	*/
		if( flags & SOIL_FLAG_TEXTURE_REPEATS )
		{
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, GL_REPEAT );
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, GL_REPEAT );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
				f->glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, GL_REPEAT );
			}
		...
		} else
		{
			...
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_S, clamp_mode );
			f->glTexParameteri( opengl_texture_type, GL_TEXTURE_WRAP_T, clamp_mode );
			if( opengl_texture_type == SOIL_TEXTURE_CUBE_MAP )
			{
				/*	SOIL_TEXTURE_WRAP_R is invalid if cubemaps aren't supported	*/
				f->glTexParameteri( opengl_texture_type, SOIL_TEXTURE_WRAP_R, clamp_mode );
			}
			check_for_GL_errors( "GL_TEXTURE_WRAP_*" );
		}
		/*	done	*/
	...
	)
{
	...
	QOpenGLFunctions *f = QOpenGLContext::currentContext()->functions();
    f->glReadPixels (x, y, width, height, GL_RGB, GL_UNSIGNED_BYTE, pixel_data);
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		QOpenGLFunctions *f = QOpenGLContext::currentContext()->functions();
		if(!QOpenGLContext::currentContext()->hasExtension("GL_ARB_texture_non_power_of_two"))
		{
...
int query_tex_rectangle_capability( void )
{
	...
	if( has_tex_rectangle_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(
			(!QOpenGLContext::currentContext()->hasExtension("GL_ARB_texture_rectangle" ) )
		&&
			(!QOpenGLContext::currentContext()->hasExtension("GL_EXT_texture_rectangle"))
		&&
			(!QOpenGLContext::currentContext()->hasExtension("GL_NV_texture_rectangle"))
			)
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	if( has_cubemap_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		QOpenGLFunctions *f = QOpenGLContext::currentContext()->functions();
		if(
			(!QOpenGLContext::currentContext()->hasExtension("GL_ARB_texture_cube_map"))
		&&
			(!QOpenGLContext::currentContext()->hasExtension("GL_EXT_texture_cube_map"))
			)
		{
			/*	not there, flag the failure	*/
